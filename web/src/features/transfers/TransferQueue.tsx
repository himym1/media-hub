  ...
  return (
    <section className="workspace-view">
      <div className="workspace-heading">
        <div><p className="eyebrow">TRANSFER QUEUE</p><h1>转存任务</h1></div>
          ...
          <div className="task-list" role="list">
            {jobs.map((job) => (
              <button
                ...
                className={selectedID === job.id ? 'task-row selected' : 'task-row'}
                key={job.id}
          ...
          <aside className="task-detail" aria-label="任务详情">
            ...
            {detail.data ? <>
              <div className="task-detail-heading">
                <div><p className="eyebrow">TASK DETAIL</p><h2>{detail.data.title}</h2></div>
                <span className={`state-chip ${detail.data.state}`}>{stateLabel[detail.data.state]}</span>
              </div>
              ...
              <ol className="event-list">
                {detail.data.events.map((event) => (
                  <li key={event.id}>
                    {event.state === 'completed' ? <CheckCircle2 size={16} /> : event.state === 'failed' || event.state === 'needs_attention' ? <CircleAlert size={16} /> : <Clock3 size={16} />}
                    <div><strong>{stateLabel[event.state]}</strong><span>{event.message || '状态已更新'}</span><time>{timeFormatter.format(new Date(event.createdAt))}</time></div>
                  </li>
                ))}
              </ol>
